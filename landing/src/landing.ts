...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .version-tag { position: absolute; bottom: 2rem; right: 2rem; font-size: 0.75rem; color: var(--text-muted); opacity: 0.5; font-weight: 600; z-index: 20; }
...
<body>
  ...
  <script>
    ...
    async function generate() {
      ...
      let receiptData = { request_id: 'req-' + Date.now(), timestamp: new Date().toISOString(), type: type.toUpperCase(), wallet: wallet };
      try {
        ...
        if(pay === 'usdc') {
          ...
          if (!uATAInfo) throw new Error('You do not have a USDC token account. Please get some USDC first.');
    ...
    function showReceipt(data) {
      ...
      content.innerHTML = \`
        ...
        <div class="receipt-section"><div class="receipt-section-title">Payment</div><div class="receipt-row"><span class="receipt-label">Method</span><span class="receipt-value">\\\${data.payment.method}</span></div><div class="receipt-row"><span class="receipt-label">Amount</span><span class="receipt-value">\\\${data.payment.amount}</span></div><div class="receipt-row"><span class="receipt-label">TX</span><a class="receipt-link" href="https://solscan.io/tx/\\\${data.payment.tx_signature}" target="_blank">\\\${data.payment.tx_signature.slice(0,8)}...\\\${data.payment.tx_signature.slice(-6)}<iconify-icon icon="ph:arrow-square-out"></iconify-icon></a></div></div>
